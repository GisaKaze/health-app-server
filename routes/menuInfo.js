...
router.post("/load", async (req, res) => { // use async/await to ensure request is fulfilled before writing to DB
    ...
    try { //parse outer xml
        const response = await fetch(DINING_API_URL_DINING_COURTS);
        if (!response.ok) {
    ...
    for (const diningCourt of DINING_COURTS) {
        const outerUrl = DINING_API_URL_DINING_COURTS + diningCourt + "/" + today;
        try {
            ...
            for (const meal of outerJson.Meals) {
                ...
                for (const station of meal.Stations) {
                    ...
                    for (const item of station.Items) {
                        const parseUrl = DINING_API_URL_MENU_ITEMS + item.ID;
